...
struct nvpower *demo_nvcpufreq(void);
//struct nvpower *nvcpu,*nvip;
		...
		unsigned long event, void *hcpu)
{
	...
	if (event == CPU_ONLINE || event == CPU_DEAD) {
		...
		nvip.nv_cpufreq = cpufreq_quick_get(0);
      		nvip.nv_cpus = num_online_cpus();

		pr_info("file:%s ,function:%s ,cpus:%d ,cpufreq:%d line no:%d \n",__FILE__,__FUNCTION__,nvip.nv_cpus,nvip.nv_cpufreq,__LINE__);
...
struct nvpower nvcpu;

	nvcpu.nv_cpufreq =cpufreq_quick_get(0);

	nvcpu.nv_cpus =num_online_cpus();

pr_info("file:%s ,function:%s ,cpus:%d ,cpufreq:%d line no:%d \n",__FILE__,__FUNCTION__,nvcpu.nv_cpus,nvcpu.nv_cpufreq,__LINE__);
